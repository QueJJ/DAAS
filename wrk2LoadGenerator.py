...
class Wrk2LoadGeneratorAsyn:
    ...
    async def generate_load(self):
        ...
        result_error = stderr.decode()

        # extract the number of requests, average latency, and timeout requests from the output
        match_requests = re.search(r"(\d+)\srequests", result_output)
        
        self.total_requests = int(match_requests.group(1)) if match_requests else 0

        match_latency = re.search(r"Latency\s+(\d+\.\d+)", result_output)
        self.avg_latency = float(match_latency.group(1)) if match_latency else 0

        match_timeouts = re.search(r"timeout\s+(\d+)", result_output)
        self.timeout_requests = int(match_timeouts.group(1)) if match_timeouts else 0
